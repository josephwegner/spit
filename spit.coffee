...
class window.Spit
  ...
  constructor: (@options) ->
    ...
    if !@options.keen_write_key? or @options.keen_write_key == ""
      throw "You have to provide a Keen.IO write key!"

    if !@options.keen_project_id? or @options.keen_project_id == ""
      throw "You have to provide a Keen.IO write key!"
  ...
  initializeKeen: ->
    ...
    @keen.setGlobalProperties @keenGlobals.bind(this)

  keenGlobals: (eventCollection) ->
    properties =
      test: @options.name
  ...
  getVariation: ->
    currentVariant = @getCookie "spit_variant"

    if currentVariant
      for variant in @options.variations
        if variant.name == currentVariant
          @variation = variant

    if !@variation?
      @variation = @options.variations[Math.floor(Math.random() * @options.variations.length)]
      @setCookie "spit_variant", @variation.name

  setUUID: ->
    @uuid = @getCookie "spit_uuid"
    ...
    if !@uuid
      ...
      @setCookie "spit_uuid", @uuid, false

  loadCSS: ->
    if @options.css? and typeof(@options.css) == "object" and @options.css.length?
      for style in @options.css
        link = document.createElement "link"
        link.type = "text/css"
        link.rel = "stylesheet"
        link.href = @variantPath style

        document.head.appendChild link

  loadJS: ->
    if @options.js? and typeof(@options.js) == "object" and @options.js.length?
      for js in @options.js
        script = document.createElement "script"
        script.type = "text/javascript"
        script.src = @variantPath js
  ...
  action: (label) ->
    ...
    @sendEvent
      ...
      event: "action"
      label: label
      completedGoal: @goalCompleted

  sendEvent: (evnt) ->
    @keen.addEvent "ab_event", evnt
  ...
  setCookie: (name, value, expiresDate = true) ->
    if expiresDate == true
    ...
    document.cookie = "#{name}=#{value}#{expiresDate}"
